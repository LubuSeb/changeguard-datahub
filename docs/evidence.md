...
| Requires no paid credentials | Demo gateway is default; no LLM dependency |
...
| Verifies the official integration | Repeatable `npm run test:live` against the local official MCP server; evidence in `docs/live-integration-evidence-20260716.json` |
...
- The live adapter has been exercised against the official local DataHub MCP server with five seeded datasets and four column-lineage edges.
